...
- **Rendered text is numbers too — and the tree is NOT a record of it.** `{"element":{"id":"cta"},"text":"CONTINUE"}` asks what accessibility is TOLD; on iOS SwiftUI collapses a card or button into one node carrying an authored a11y summary, so the visible string can be absent entirely (measured: `credit_select` exposes `'To account'` while rendering `'Select credit account'`; the visible `CONTINUE` is nowhere in the tree). To check what the USER SEES, use `{"element":{"id":"cta"},"ocr":{"text":"CONTINUE"}}` — it reads the string back off the screenshot. Whole screen: contract anchors carrying `text` (exact rendered string) or `text_dynamic: true` (amounts/dates — locale formatting differs legitimately, never literal-diffed) add a `## text parity` table. Its `src` column says whether a row came from `ocr` or the `tree` fallback (macOS-only recognizer); rows never mix the two across platforms. **Type size rides along:** `ocr` returns rendered ink height, compared android-vs-ios at `tolerance_size_pct` (default 10) and only where both strings match — a matching CTA reads 0.74% apart, the real 22sp-vs-17pt title drift 12.63%. Single element: `"ocr":{"heightPct":2.96}`. Placeholders must be asserted in the EMPTY state — iOS drops the placeholder node once a field is filled. An `OCCLUDED`/UNREAD row is NOT copy drift: something is drawn over the rect (an IME usually) **or** the text is at a contrast the recognizer cannot resolve, which WOULD be a finding — look at the screenshot before dismissing it.
