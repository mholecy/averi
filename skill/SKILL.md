...
- **Selector values with spaces are double-quoted**: `text:"Sign in"` (exact) or `text~"Sign in"` (regex). An unquoted `text:Sign in` parses `text:Sign` and fails at `"in"` — the error says so and suggests the quoted form (measured 2026-09-17: three dead calls before the caller guessed). `~` is unanchored — `text~"Sign in"` also matches `Sign in later` — so use `:` when you mean the whole label.
- **An Android tree read that returns no XML or times out names the DEVICE state, not uiautomator.** `device <id> is not reachable: adb get-state says "device offline"` → recover the emulator (wait for `adb devices` to read `device`; `adb kill-server && adb start-server` if it does not). Not an averi bug, not an app defect, and never the moment to call `ensure_state`: its `detect` would read nothing and the ladder would escalate into a re-registration. `is still settling` (uiautomator's null root right after a launch; `ui_snapshot`/`tap` retry once for you) → `screenshot` (it waits for stability), then retry. `is reachable but SLOW` (the dump timed out) → the host is under load; not a dead app. `appAlive: unknown` means the device could not be ASKED (adb/simctl timeout under host load, device offline) — it is not a crash.
- **Reading a failing flow's trace:** it ends with a `✗ <step> — failed — <reason>` line naming the step that failed — read that line, not the last successful one (steps log only on success, so before this line existed the last line was the step BEFORE the culprit). Inside an `ensure_state` ladder a `✗` before a `⚠ reach <flow>` line says which step of that rung failed, and the ladder went on — it is not the headline error. A `⚠ fill … typing APPENDS` line means a masked field already held text and `clear` was off: the length check passed, the content may still be wrong.
...
- **Geometry is checked with numbers — never eyeball margins from screenshots.** One element: `assert` with `{"element":{"id":"card"},"rect":{"x":24,"w":345,"h":129,"frameWidth":393}}` (Figma-frame units, deltas in % of screen width; `y` is reported but never fails — absolute y drifts with device aspect ratio, vertical position is judged by gaps). Whole screen: `verify` with `contract: path/to/layout-contract.json` — appends a `## rect parity` per-anchor table (deltas vs contract and android-vs-ios, gap and aspect rows, MISSING anchors). The `aspect` row compares SHAPE android-vs-ios, and **omitting `w`/`h` does not switch it off** — omission means "not pinned", and the row's value is catching shape bugs on a side nobody pinned. Where the two platforms genuinely derive a side by different mechanisms (measured: Android's a11y touch-target floor inflates a header's node box to 48 where iOS reports 44 — a 10.73% aspect spread against correct code), set `aspect: false` on that anchor with the reason beside it; `tolerance_aspect_pct` is a separate threshold from `tolerance_pct` because a ratio spread is not a %-of-screen-width delta. If the output warns the width is a CONTENT width (widest rect starts inset), every delta is scaled wrong — on iOS set `app.ios.treeSource: wda` (idb's flat source often has no window rect to find); don't quote those numbers. **Fills are numbers too:** one element: `assert` with `{"element":{"id":"card"},"color":{"expected":"#FDFDFD","deltaE":8}}` (CIEDE2000 over the element's sampled region; hex only — token names resolve in the superrepo layer, never here). Whole screen: contract anchors carrying `bg`/`bg_dark`/`sample` add a `## color parity` table to `verify` (android-vs-ios primary at `tolerance_de` 8, vs-contract at 1.5×). A single-platform run only gets the looser vs-contract axis and at defaults MISSES real bugs — the 2026-08-13 grey-card bug measured dE00 10.19, under the default 12 — so set `tolerance_de: 6` in the contract when only one platform is in the loop. Thin 1–2 px strokes (borders) are invisible to region sampling — those stay with the screenshot judge.
- Selectors: prefer `id:` (stable), then `label:`/`text:`, then `role:` combinations. `ui_snapshot(platform, filter)` shows you what's there.
- If an expected element is missing or below the fold, use `scroll_until(platform, selector)` (or the `scroll_until:` flow step) — element-targeted, no coordinates, portable. **"Visible" means INTERSECTS the viewport, so the element may still be clipped at an edge** — the reply names the clipped edges and the visible fraction, and in a flow trace a clipped stop is marked `⚠`. When the next step MEASURES the element (a `rect` assert, a screenshot), pass `fully: true`: a clipped rect measures the clipped box, and `fully: true` fails naming the shortfall instead. Measured 2026-08-27: a row clipped by a floating bottom-nav bar reported a bare "visible", the following assert read `h 143` against a pinned `60`, and the investigation went at the app's row-height logic — which was correct; the scroll container had no clearance. Raw `swipe` remains for gestures that aren't "bring X into view" (its coordinates are per-platform units).
...
flows:
  login:
    destructive: true   # optional: never repeat this flow in a recovery pass.
                        # Inferred for `clearState: true`; set it by hand for
                        # wipes averi cannot see — a logout, a one-shot SMS
                        # code, anything deleted server-side.
    steps:
...
`reach:` is tried in order and `detect` is re-checked after each flow, so the rest are skipped once the state is reached. A rung that FAILS escalates to the next one rather than failing the call — it shows up as `⚠ reach <flow>` in the trace, and if you see one, fix that flow: it is doing nothing but costing time. Put the cheap, idempotent recovery first and the destructive one last — `reach: [dismiss_post_login_prompts, login]` — and note what "destructive" means here: `launch { clearState: true }` deletes the app's data container (iOS) / runs `pm clear` (Android), so everything the app persisted goes with it — on apps that bind to the device, that is a backend re-registration per wipe. averi logs a `⚠ reach <flow>: this rung is DESTRUCTIVE` line BEFORE running such a rung, and a `⚠ clearState` line with a running count after it executes; if you see that count climbing while you debug selectors, stop and re-read the trace instead. **Model the recoverable logged-out screens too.** An app with a session inactivity timeout lands on a "you have been logged out" screen from which the device registration is still valid and a PIN login restores it — but if no state declares that screen, `logged_in`'s `detect` correctly says "not logged in", the ladder escalates correctly, and the correct escalation is the maximally expensive one. A long gap between the two legs of a cross-platform run makes this the normal case, not an edge case: measured 2026-08-27, ~30 minutes on the other platform cost a full re-registration that a `session_expired` state with a PIN-login `reach`, declared before `login`, would have avoided. Rung ORDER is the only expression of preference — averi will not reorder the ladder for you, only warn before it spends. A rung that fails because of a CONFIG mistake (undeclared credential, unset `${VAR}`) aborts the ladder instead of escalating — the next flow cannot fix it and would only cost you a wipe.

If the state is still not reached after the last rung — whether that rung completed or threw, e.g. on its own trailing `wait:` — averi re-runs the EARLIER rungs once before failing (`↻ recovery <state>` in the trace). This is for the late-interstitial case: `login` completes, a network-gated biometrics offer arrives after login's own `optional:` windows closed, and `dismiss_post_login_prompts` — the exact cure — has already been consumed. Only rungs provably safe to repeat take part; the last rung, anything containing `launch { clearState: true }` (including inside `branch`/`optional`, or reachable via its `requires:`), and anything marked `destructive: true` are never re-run, so **the recovery pass adds no wipes** — what a call costs is what the forward ladder already spent (which can be more than one, if a rung's `requires:` chain wipes too). Lines beginning `⚠ recovery <flow>` or `⚠ salvage <flow>` are that last-ditch attempt reporting its own stumbles — they are diagnostics, never the reason a call failed, so read the headline error rather than acting on them. Treat the pass as a safety net, not a design: a `↻ recovery` line in a passing trace means the flow that produced it should get a `timeout:` on its optional tap, or the post-login fork should be modelled as a state with `any:` over both outcomes (below).

Steps: `launch`, `tap` (element spec + optional `timeout:` overriding the default find-and-settle budget), `type`, `type_pin` (`twice:` for set+confirm; `keypad:` takes `id_pattern` or — for keypads without resource-ids, common in Compose — `text_pattern: "{digit}"`), `fill` (element spec + `value`, opt-in `clear`), `swipe` (`direction`, `times`), `scroll_until` (`element`, optional `direction`/`maxSwipes`/`timeout`/`fully` — swipe until visible; `fully: true` demands the element ENTIRELY inside the viewport, not merely overlapping), `assert` (inline element asserts; a failure fails the flow), `wait` (element/state), `branch` (first matching `when` wins), `optional` (absence is fine), and per-platform overrides (`android:`/`ios:` on one step).
